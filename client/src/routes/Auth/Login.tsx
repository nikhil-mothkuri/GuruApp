      ...
      <div className="w-full max-w-sm">
        ...
        <div className="border border-[#dadce0] rounded-3xl px-8 py-10 bg-white shadow-sm">
          ...
          <div className="text-center mb-6">
            <Logo />
            <h1 className="text-2xl font-normal text-[#111827] mt-6">{t('login.title')}</h1>
            <p className="text-[#475569] text-sm mt-2">{t('login.subtitle')}</p>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            {login.error && (
              <p className="text-[#d93025] text-sm bg-[#fce8e6] px-3 py-2 rounded">
                {(login.error as { response?: { data?: { error?: { message?: string } } } })
                  .response?.data?.error?.message ?? t('login.failed')}
              </p>
            )}
            ...
            <div className="flex items-center justify-between pt-2">
              ...
              >
                {login.isPending ? t('login.signingIn') : t('login.next')}
              </button>
            </div>
          </form>
          ...
          <div className="relative my-6">
            ...
            <div className="relative flex justify-center">
              <span className="bg-white px-3 text-xs text-[#5f6368]">{t('login.or')}</span>
            </div>
          </div>
