      ...
      <div className="w-full max-w-sm">
        ...
        <div className="border border-[#dadce0] rounded-3xl px-8 py-10 bg-white shadow-sm">
          ...
          <div className="text-center mb-6">
            <Logo />
            <h1 className="text-2xl font-normal text-[#111827] mt-6">Create your account</h1>
            <p className="text-[#475569] text-sm mt-2">Join SakshamBharat to learn, sell, and grow.</p>
          </div>

          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div>
              <label className="block text-sm text-[#202124] mb-1.5 font-medium">Full name</label>
              <input
                {...register('name')}
                autoComplete="name"
            ...
            {/* Role selection */}
            <div>
              <p className="text-sm text-[#202124] mb-2 font-medium">I want to join as</p>
              <div className="grid grid-cols-2 gap-2">
                <label className={`flex flex-col items-center gap-2 py-3 rounded border-2 cursor-pointer transition-all ${isStudent ? 'border-[#1a73e8] bg-[#e8f0fe]' : 'border-[#dadce0] hover:border-[#aaa]'}`}>
                  <input type="checkbox" {...register('isStudent')} className="sr-only" />
                  <span className="text-xl">🎓</span>
                  <span className="text-xs font-medium text-[#202124]">Student</span>
                </label>
                <label className={`flex flex-col items-center gap-2 py-3 rounded border-2 cursor-pointer transition-all ${isGuru ? 'border-[#1a73e8] bg-[#e8f0fe]' : 'border-[#dadce0] hover:border-[#aaa]'}`}>
                  <input type="checkbox" {...register('isGuru')} className="sr-only" />
                  <span className="text-xl">🧑‍🏫</span>
                  <span className="text-xs font-medium text-[#202124]">Guru</span>
                </label>
              </div>
              <p className="text-xs text-[#5f6368] mt-1.5">You can be both a Guru and a Student.</p>
            </div>

            {signup.error && (
              <p className="text-[#d93025] text-sm bg-[#fce8e6] px-3 py-2 rounded">
                {(signup.error as { response?: { data?: { error?: { message?: string } } } }).response?.data?.error?.message ?? 'Sign up failed'}
              </p>
            )}
