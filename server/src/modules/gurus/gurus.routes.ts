...
import { updateGuruProfileSchema, addSkillSchema, addVideoSchema, guruSearchSchema, createSlotSchema, updateSlotSchema } from '@guruapp/shared';
...
// Guru slot management (must come before /:id catch-all)
gurusRouter.get('/me/slots', authenticate, requireGuru(), slotsController.getMySlots);
gurusRouter.post('/me/slots', authenticate, requireGuru(), validate(createSlotSchema), slotsController.create);
gurusRouter.put('/me/slots/:slotId', authenticate, requireGuru(), validate(updateSlotSchema), slotsController.update);
...
gurusRouter.put('/me/profile', authenticate, requireGuru(), validate(updateGuruProfileSchema), gurusController.upsertMyProfile);
gurusRouter.post('/me/skills', authenticate, requireGuru(), validate(addSkillSchema), gurusController.addSkill);
gurusRouter.delete('/me/skills/:skillId', authenticate, requireGuru(), gurusController.deleteSkill);
gurusRouter.post('/me/photos', authenticate, requireGuru(), upload.single('photo'), gurusController.addPhoto);
gurusRouter.delete('/me/photos/:photoId', authenticate, requireGuru(), gurusController.deletePhoto);
gurusRouter.post('/me/videos', authenticate, requireGuru(), validate(addVideoSchema), gurusController.addVideo);
