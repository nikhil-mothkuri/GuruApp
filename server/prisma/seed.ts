...
async function main() {
  ...
  const guru1 = await prisma.user.upsert({
    ...
    update: {},
    create: {
      email: 'alice@guruapp.com',
      passwordHash,
      name: 'Alice Sharma',
      isGuru: true,
      isStudent: false,
      bio: 'Yoga & meditation instructor with 10 years experience.',
    },
  });
  ...
  const guru2 = await prisma.user.upsert({
    ...
    update: {},
    create: {
      email: 'bob@guruapp.com',
      passwordHash,
      name: 'Bob Chen',
      isGuru: true,
      isStudent: true,
      bio: 'Professional piano teacher and music theorist.',
    },
  });
  ...
  const profile1 = await prisma.guruProfile.upsert({
    ...
    create: {
      userId: guru1.id,
      tagline: 'Transform your mind and body through yoga',
      ratingAvg: 4.8,
      ratingCount: 24,
    },
  });
  ...
  const profile2 = await prisma.guruProfile.upsert({
    ...
    create: {
      userId: guru2.id,
      tagline: 'Learn piano from beginner to advanced',
      ratingAvg: 4.5,
      ratingCount: 12,
    },
  });
